package br.com.enginer.domain.system.usercase.tag;
 ...
 * 
 */
public class TagUserCase extends AbstractUserCase<Tag> implements br.com.enginer.domain.system.usercase.port.tag.TagUserCase {
	...
	public void pull(String domain, Object domainId, List<String> tags) {
		...
		Optional.ofNullable(tags).orElse(List.of()).stream().filter(Objects::nonNull).map(String::trim)
				.filter(s -> !s.isEmpty()).forEach(name -> {
